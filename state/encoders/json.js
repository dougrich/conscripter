export default {
  ...
  encode: ({ fonts: { substitutions, fontname }}) => {
    return 'data:text/json;charset=utf-8;base64,' + btoa(JSON.stringify({ version: '0.1.1', substitutions, fontname }))
  },
  decode: (data) => {
    if (data[0] === '{') {
      ...
      const { substitutions = [], fontname = 'My Custom Font' } = JSON.parse(atob(data.slice('data:text/json;charset=utf-8;base64,'.length)))
      return {
        substitutions,
        fontname
      }
